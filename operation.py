...
def plant_algorithm(h_i, h_o, t, status, control, turbine_specs, sluice_specs, flux_limiter=0.2):
    """
    ...
    Pumping parameters based on Yates et al., 2013 - https://doi.org/10.1016/j.renene.2013.01.039
    ...
    # Special case - trigger end of pumping
    ...
    if status["m"] == 2:  # Generating            HW -> LW
        status["P"] = status["f_r"] * control["N_t"] * turbine_specs["eta"][0] * \
                      turbine_parametrisation(abs(status["DZ"]),
                                              turbine_specs)[0]
    ...
    if status["m"] == 3:  # Generating/sluicing    HW -> LW
        status["P"] = control["N_t"] * turbine_parametrisation(abs(status["DZ"]), turbine_specs)[0] * \
                      turbine_specs["eta"][0]
...
def operational_model(simulation, elev_time, area_elev, status, control, params, export_output=None):
    """
    Timestepper for the operational model (first order accuracy). Interpolates elevation and area and calls
    plant_operation

    :param simulation: simulation characteristics (starting time, simulation duration, dt
    :param elev_time: elevation-time signal
    :param area_elev: area-elevation signal
    :param status: lagoon status at the beginning
    :param control: power plant control parameters
    :param params: hydraulic structure parameters
    :param export_output:
    """
    ...
    for step in range(0, int(simulation["t"] / simulation["Dt"]), 1):
        ...
        if export_output:
            ...
            plant_operation(step * simulation["Dt"] + simulation["start_t"], simulation["Dt"], h_inner,
